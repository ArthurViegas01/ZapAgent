...
async def test_schedule_appointment_creates_id_and_confirmation(base_state: AgentState) -> None:
    diff = await schedule_appointment(base_state)
...
async def test_schedule_appointment_uses_existing_draft(base_state: AgentState) -> None:
    ...
    assert "01/05" in diff["response"] and "10:00" in diff["response"]
